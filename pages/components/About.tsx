...
const About = () => {
  ...
  return (
    <div id="About" className="px-8 md:px-32 pb-32 content-center bg-gradient-to-r from-green-300 via-teal-500 to-violet-500">
      <h1 className="pt-14 font-bold text-center text-white text-bold text-4xl drop-shadow-md">{about.title}</h1>
      <div className="mt-16 flex flex-col align-center items-center">
        <div className="pt-8 md:py-0 md:w-1/2 text-white md:ml-4 text-center flex flex-col items-center">
          <div className="about__primary bg-white bg-opacity-25 p-5 rounded shadow-md">
            <span>{about.primary}</span>
          </div>
          <div className="mt-6 bg-white bg-opacity-25 p-5 rounded shadow-md">
            <span>{about.secondary}</span>
          </div>
          <a target="_blank" href={about.resume} className="about__resume text-white text-l mt-6 mx-auto md:mx-0 border-2 border-white py-1 px-2 w-36 text-center hover:underline transition-all rounded">View Resume</a>
        </div>
      </div>
    </div>
  );
